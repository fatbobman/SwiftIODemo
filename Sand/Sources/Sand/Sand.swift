...
public final class Sand<S, A> where S: ST7789Base, A: AnalogInBase {
  ...
  public init(screen: S, cursor: A, getSystemUptimeInMilliseconds: @escaping () -> Int64) {
    self.screen = screen
    self.getSystemUptimeInMilliseconds = getSystemUptimeInMilliseconds
    ...
    drawEmptyRect(at: (cursorPos * sandSize, 0), width: sandSize, height: sandSize, stroke: 1, color: 0xFFFF)
  ...
  func updateCursor(cursor: A) {
    cursorPos = getCursorPos(cursor: cursor)

    if cursorPos != lastCursorPos {
      drawRect(at: (lastCursorPos * sandSize, 0), width: sandSize, height: sandSize, color: 0)
      drawEmptyRect(at: (cursorPos * sandSize, 0), width: sandSize, height: sandSize, stroke: 1, color: 0xFFFF)
      lastCursorPos = cursorPos
    }
  }
  ...
  func getCursorPos(cursor: A) -> Int {
    var values: Float = 0
        ...
        for x in gridColors[y].indices {
          ...
          if color > 0 {
            ...
            if gridColors[y + 1][x] == 0 {
              ...
              drawRect(at: (x * sandSize, y * sandSize), width: sandSize, height: sandSize, color: 0)
              drawRect(at: (x * sandSize, (y + 1) * sandSize), width: sandSize, height: sandSize, color: color)
            } else {
              ...
              if left, x > 0, gridColors[y + 1][x - 1] == 0 {
                ...
                drawRect(at: (x * sandSize, y * sandSize), width: sandSize, height: sandSize, color: 0)
                drawRect(at: ((x - 1) * sandSize, (y + 1) * sandSize), width: sandSize, height: sandSize, color: color)
  ...
  func drawEmptyRect(at point: Point, width: Int, height: Int, stroke: Int, color: UInt16) {
    for w in 0 ..< width {
      for line in 0 ..< stroke {
        screen.writePixel(x: point.x + w, y: point.y + line, color: color)
        screen.writePixel(x: point.x + w, y: point.y + height - stroke + line, color: color)
      }
    }

    for h in stroke ..< height - stroke {
      for line in 0 ..< stroke {
        screen.writePixel(x: point.x + line, y: point.y + h, color: color)
        screen.writePixel(x: point.x + width - stroke + line, y: point.y + h, color: color)
      }
    }
  }
}
